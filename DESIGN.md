...
| M8 | ✅ Multiple layers (Ground + Overlay). Tiles are assigned to a layer; each layer is solved independently with WFC in its own dense tile-index space (overlay adds a synthetic EMPTY tile so it stays sparse), adjacency inferred per layer from that layer's painted example. Rules tab paints each layer with the ground shown beneath for context; results composite to the PNG and export as one CSV per layer. Browser + unit verified. (Deeper: N layers, layered `.tmx`.) |
