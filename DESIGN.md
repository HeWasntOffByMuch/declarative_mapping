...
| M10 | ✅ **Multiple tilesets** (shared tile size) feeding one merged catalog — each tile carries a `sourceId`; add sheets in the Tiles tab, rendering picks the right atlas per tile, Rules palette can filter by sheet. **Fuzzy search** in the tile catalog dims non-matching tiles. Project files v4 (atlases[]), migrating single-atlas ≤v3. Browser + unit verified. |
| M9 | ✅ Multiple **named examples** ("dungeon", "forest", …), each dual-layer. Rules tab adds/renames/deletes examples; generation pools N×N patterns across all examples per layer, so focused small examples combine into one richer model. Resize preserves painting. Project files v3 (examples[]), migrating v1/v2. Browser + unit verified. Future: a separate structural dungeon generator that feeds guaranteed rooms into this WFC+LLM tail. |
